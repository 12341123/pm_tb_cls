...
def add_cfg(cfg):
    cfg.MODEL.BCE=CN()
    cfg.MODEL.BCE.INPUTFEATURESIZE = 768
    cfg.MODEL.BCE.MLPFEATURESIZE =384
    ...
    # cfg.INPUT.RANDOM_COLOR_JITTER.ENABLED = True
def add_swint_config(cfg):
    # SwinT backbone
    cfg.MODEL.SWINT = CN()
    cfg.MODEL.SWINT.EMBED_DIM = 96
    cfg.MODEL.SWINT.OUT_FEATURES = ["stage5",]
    cfg.MODEL.SWINT.DEPTHS = [2, 2, 6, 2] #SwinB [2,2,18,2] satge5 1024
...
def setup(args):
    ...
    """
    cfg = get_cfg()
    add_swint_config(cfg)#SwinT Config
    add_cfg(cfg)
...
def main(args):
    with open('/ssd2/wangzd/194/194_bbox_train.json') as train:
    ...
    all_val=val_dict194+val_dict206+fp_dict_val
    ...
    DatasetCatalog.register('fp_train',get_fp_train)
